...
ChannelService.prototype.broadcast = function(stype, route, msg, opts, cb) {
  console.log("broadcast:",stype, route, msg);
  if(cb) { cb()};
};
...
var sendMessageByGroup = function(channelService, route, msg, groups, opts, cb) {
  console.log("sendMessageByGroup:", route, msg, groups);
  if(cb) {cb()};
};
...
var genKey = function(self, name) {
  if(!!name) {
    return self.prefix + ':testServerId:' + name;
  } else {
    return self.prefix + ':testServerId';
  }
};
